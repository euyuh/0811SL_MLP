...
import os
import pandas as pd
from quant_utils import register_activation_ema_hooks
...
from quant_utils import (
    ...
    inference_quantized, export_quantized_model, FakeQuantLinear, ACT_EMA
)
import matplotlib
matplotlib.use('Agg')
...
if isinstance(net_glob_client, torch.nn.DataParallel):
    register_activation_ema_hooks(net_glob_client.module, prefix="client")
else:
    register_activation_ema_hooks(net_glob_client, prefix="client")

# 服务器模型注册
if isinstance(net_glob_server, torch.nn.DataParallel):
    register_activation_ema_hooks(net_glob_server.module, prefix="server")
else:
